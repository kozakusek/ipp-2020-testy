#include <stdint.h>
#include <stdlib.h>
#include <assert.h>
#include <stdio.h>
#include "gamma.h"
#include <stdbool.h>


int main() {

/*
scenario: fill_board_without_collisions
uuid: 428125409
*/
/*
gamma_move, fill board no collisions, many players, unlimited areas
*/
gamma_t* board = gamma_new(10, 10, 30, 100);
assert( board != NULL );


assert( gamma_move(board, 24, 9, 5) == 1 );
assert( gamma_move(board, 11, 1, 4) == 1 );
assert( gamma_move(board, 27, 4, 4) == 1 );
assert( gamma_move(board, 24, 1, 0) == 1 );
assert( gamma_move(board, 17, 7, 0) == 1 );
assert( gamma_move(board, 24, 6, 8) == 1 );
assert( gamma_move(board, 24, 9, 6) == 1 );
assert( gamma_move(board, 15, 4, 9) == 1 );
assert( gamma_move(board, 22, 0, 1) == 1 );
assert( gamma_move(board, 24, 3, 7) == 1 );
assert( gamma_move(board, 11, 4, 1) == 1 );
assert( gamma_move(board, 20, 3, 9) == 1 );
assert( gamma_move(board, 28, 8, 3) == 1 );
assert( gamma_move(board, 27, 5, 8) == 1 );
assert( gamma_move(board, 12, 5, 2) == 1 );
assert( gamma_move(board, 13, 6, 2) == 1 );
assert( gamma_move(board, 15, 7, 1) == 1 );
assert( gamma_move(board, 12, 2, 9) == 1 );
assert( gamma_move(board, 22, 9, 4) == 1 );
assert( gamma_move(board, 24, 0, 0) == 1 );
assert( gamma_move(board, 10, 3, 5) == 1 );
assert( gamma_move(board, 30, 6, 3) == 1 );
assert( gamma_move(board, 22, 6, 0) == 1 );
assert( gamma_move(board, 9, 6, 4) == 1 );
assert( gamma_move(board, 23, 4, 6) == 1 );
assert( gamma_move(board, 24, 1, 1) == 1 );
assert( gamma_move(board, 16, 1, 6) == 1 );
assert( gamma_move(board, 23, 5, 4) == 1 );
assert( gamma_move(board, 14, 1, 8) == 1 );
assert( gamma_move(board, 7, 8, 9) == 1 );
assert( gamma_move(board, 6, 4, 2) == 1 );
assert( gamma_move(board, 11, 2, 8) == 1 );
assert( gamma_move(board, 2, 2, 1) == 1 );
assert( gamma_move(board, 19, 3, 6) == 1 );
assert( gamma_move(board, 10, 8, 8) == 1 );
assert( gamma_move(board, 14, 0, 8) == 1 );
assert( gamma_move(board, 27, 0, 5) == 1 );
assert( gamma_move(board, 27, 6, 9) == 1 );
assert( gamma_move(board, 26, 9, 0) == 1 );
assert( gamma_move(board, 14, 4, 8) == 1 );
assert( gamma_move(board, 3, 9, 8) == 1 );
assert( gamma_move(board, 1, 7, 5) == 1 );
assert( gamma_move(board, 18, 5, 3) == 1 );
assert( gamma_move(board, 8, 4, 5) == 1 );
assert( gamma_move(board, 4, 5, 7) == 1 );
assert( gamma_move(board, 22, 2, 3) == 1 );
assert( gamma_move(board, 10, 6, 1) == 1 );
assert( gamma_move(board, 2, 8, 0) == 1 );
assert( gamma_move(board, 9, 2, 0) == 1 );
assert( gamma_move(board, 8, 8, 4) == 1 );
assert( gamma_move(board, 15, 3, 2) == 1 );
assert( gamma_move(board, 2, 0, 4) == 1 );
assert( gamma_move(board, 25, 7, 3) == 1 );
assert( gamma_move(board, 28, 9, 1) == 1 );
assert( gamma_move(board, 2, 8, 7) == 1 );
assert( gamma_move(board, 9, 2, 6) == 1 );
assert( gamma_move(board, 17, 8, 1) == 1 );
assert( gamma_move(board, 10, 2, 7) == 1 );
assert( gamma_move(board, 4, 3, 3) == 1 );
assert( gamma_move(board, 13, 5, 5) == 1 );
assert( gamma_move(board, 29, 6, 6) == 1 );
assert( gamma_move(board, 14, 4, 3) == 1 );
assert( gamma_move(board, 4, 3, 8) == 1 );
assert( gamma_move(board, 21, 0, 9) == 1 );
assert( gamma_move(board, 6, 7, 6) == 1 );
assert( gamma_move(board, 20, 7, 2) == 1 );
assert( gamma_move(board, 10, 5, 0) == 1 );
assert( gamma_move(board, 9, 9, 9) == 1 );
assert( gamma_move(board, 18, 6, 7) == 1 );
assert( gamma_move(board, 12, 8, 6) == 1 );
assert( gamma_move(board, 1, 5, 6) == 1 );
assert( gamma_move(board, 21, 1, 9) == 1 );
assert( gamma_move(board, 6, 3, 4) == 1 );
assert( gamma_move(board, 29, 1, 7) == 1 );
assert( gamma_move(board, 14, 5, 1) == 1 );
assert( gamma_move(board, 7, 1, 3) == 1 );
assert( gamma_move(board, 1, 1, 2) == 1 );
assert( gamma_move(board, 30, 9, 2) == 1 );
assert( gamma_move(board, 23, 7, 7) == 1 );
assert( gamma_move(board, 3, 0, 3) == 1 );
assert( gamma_move(board, 10, 9, 3) == 1 );
assert( gamma_move(board, 30, 6, 5) == 1 );
assert( gamma_move(board, 7, 4, 7) == 1 );
assert( gamma_move(board, 8, 4, 0) == 1 );
assert( gamma_move(board, 2, 5, 9) == 1 );
assert( gamma_move(board, 9, 9, 7) == 1 );
assert( gamma_move(board, 19, 3, 0) == 1 );
assert( gamma_move(board, 1, 2, 4) == 1 );
assert( gamma_move(board, 2, 7, 4) == 1 );
assert( gamma_move(board, 21, 0, 6) == 1 );
assert( gamma_move(board, 6, 8, 2) == 1 );
assert( gamma_move(board, 18, 7, 8) == 1 );
assert( gamma_move(board, 22, 1, 5) == 1 );
assert( gamma_move(board, 30, 2, 2) == 1 );
assert( gamma_move(board, 24, 0, 7) == 1 );
assert( gamma_move(board, 15, 2, 5) == 1 );
assert( gamma_move(board, 13, 0, 2) == 1 );
assert( gamma_move(board, 20, 7, 9) == 1 );
assert( gamma_move(board, 7, 8, 5) == 1 );
assert( gamma_move(board, 1, 3, 1) == 1 );


gamma_delete(board);

    return 0;
}
